...
class Featurizer():
    ...
    def __init__(self, atom_codes=None, atom_labels=None,
        ...
        if protein_properties:
            ...
            self.PATH = path
            pssm = ['pssm_' + str(i) for i in range(1,21)]
            self.FEATURE_NAMES += ['SS', 'Phi', 'Psi', 'RSA'] + pssm
    ...
    def load_protein_proterties(self, PDBID):
        ...
        print("==> Protein Data loaded : ", PDBID)
    ...
    def get_features(self, molecule, PDB_ID, molcode=None):
        ...
        if(self.take_protein) and PDB_ID is not None:
        ...
        for i, atom in enumerate(molecule):
            ...
            if atom.atomicnum > 1:
                ...
                if(self.take_protein):
                    np_atom_coords = (np.float32(atom.coords[0]), np.float32(atom.coords[1]), np.float32(atom.coords[2]))
                    if(np_atom_coords not in list(self.protein_property_dict.keys())):
                        #print("Not in system : ", atom.coords, PDB_ID, count)
                        count+=1
                        pass
                        ...
                        protein_prop = list(self.protein_property_dict[np_atom_coords])
